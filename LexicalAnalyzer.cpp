...
Token LexicalAnalyzer::getNumber() {
    ...
    while(!isSpace(currentChar) and currentPosition < sql_len) {
        if(currentChar == '.') {
            if(dot == 1) {
                flag = false;
            }
...
Token LexicalAnalyzer::getWords() {
    ...
    while(!isSpace(sql_str[currentPosition]) and currentPosition < sql_len) {
        if(sql_str[currentPosition] == '(' or sql_str[currentPosition] == ')'
        or sql_str[currentPosition] == '[' or sql_str[currentPosition] == ']') break;
    ...
    if(illegalCharNum) {
        return Token(TokenType::ILLEGAL, str);
    }
